...
class Material(models.Model):
    ...
    @property 
    def Stock(self):
        purchases = Transaction_Details.objects.filter(Material = self, Transaction__Transaction_Type = 'COMPRA').aggregate(total_purchased = models.Sum('Quantity'))['total_purchased'] or 0
        
        sales = Transaction_Details.objects.filter(Material = self, Transaction__Transaction_Type = 'VENTA').aggregate(total_sold = models.Sum('Quantity'))['total_sold'] or 0
